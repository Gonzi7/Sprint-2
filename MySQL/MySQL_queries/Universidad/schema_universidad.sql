...
SELECT per.nombre, per.apellido1, per.apellido2, dep.nombre AS nombre_departamento
FROM persona per
...
SELECT asig.nombre, cur.anyo_inicio, cur.anyo_fin
FROM alumno_se_matricula_asignatura asm
INNER JOIN asignatura asig
ON asm.id_asignatura = asig.id
INNER JOIN curso_escolar cur 
ON asm.id_curso_escolar = cur.id
...
SELECT DISTINCT depa.nombre
FROM departamento depa
INNER JOIN profesor prof
ON depa.id = prof.id_departamento
INNER JOIN asignatura asig 
ON prof.id_profesor = asig.id_profesor
INNER JOIN grado
ON asig.id_grado = grado.id
...
COUNT(prof.id_profesor) AS numero_profesores
FROM departamento dep
INNER JOIN profesor prof 
ON dep.id = prof.id_departamento
GROUP BY dep.nombre
...
COUNT(asig.id) AS numero_asignaturas
FROM grado 
LEFT JOIN asignatura asig ON grado.id = asig.id_grado
GROUP BY grado.nombre
ORDER BY numero_asignaturas DESC, grado.nombre;
...
SELECT grado.nombre AS nombre_grado, 
asig.tipo AS tipo_asignatura, 
SUM(asig.creditos) AS suma_creditos
FROM grado 
JOIN asignatura asig ON grado.id = asig.id_grado
GROUP BY grado.nombre, asig.tipo
ORDER BY grado.nombre, asig.tipo;
...
SELECT cur.anyo_inicio, 
COUNT(DISTINCT asm.id_alumno) AS numero_alumnos
FROM curso_escolar cur
INNER JOIN alumno_se_matricula_asignatura asm 
ON cur.id = asm.id_curso_escolar
GROUP BY cur.anyo_inicio
ORDER BY cur.anyo_inicio;
